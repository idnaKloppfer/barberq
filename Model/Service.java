...
public class Service {
	...
	public boolean insertService() {
		...
		String sql = "INSERT INTO Service(price,name,duration)" + "values(" + this.price + ",'" + this.service + "',"
				+ this.duration + ")";
	...
	public boolean updateService() {
		update = false;
		String sql = "UPDATE service set price = " + this.price + ",name = '" + this.service + "', duration = "
				+ this.duration + "" + " where id = " + this.id + "";
		int result = connection.runUpdate(sql);
	 ...
	 *         successfully or not.
	 */
	public boolean searchHairCutByNameService(String nameService) {

		boolean flag = false;
		String sql = "select * from service where name = '" + nameService + "'";
	 ...
	 * @throws SQLException
	 */
	public void addPriceServiceToArray(ListModel<String> model) throws SQLException {
	 ...
	 * @return list 
	 */
	public String[][] listServicesInCompletedMeeting(Long idMeeting) {
	 ...
	 * @return true or false if data capture
	 */
	public boolean searchServiceByName(String name) {
	 ...
	 * @return duration
	 */
	public long capturDuration(ArrayList<String> nameservice) {

		Connection conec = connection.getConec();
		long duration = 0;
		String sql = "select duration from service where name  = ?";

		PreparedStatement captur = null;

		try {
			for (int i = 0; i < nameservice.size(); i++) {
				captur = conec.prepareStatement(sql);
				captur.setString(1, nameservice.get(i));
				ResultSet result = captur.executeQuery();
				result.next();
