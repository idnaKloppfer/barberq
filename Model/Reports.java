...
//			URL employee = getClass().getResource("/View/Report/");
			...
			JasperPrint jasperPrint = JasperFillManager.fillReport(report, param, conec.getConec());
	...
	public void customersServiceDaily(String date, boolean printt) {
		try {
			...
			param.put("logo", this.getClass().getResourceAsStream(logo));
			JasperPrint recipePrint = JasperFillManager.fillReport(report, param, conec.getConec());
			Locale local = new Locale("heb", "ISR");
			...
			JOptionPane.showMessageDialog(principal,
					"The report could not be opened:\n The database is empty in finished queues");
		}
	}
